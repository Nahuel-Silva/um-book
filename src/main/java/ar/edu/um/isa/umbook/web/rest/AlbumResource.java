...
/**
 * REST controller for managing {@link ar.edu.um.isa.umbook.domain.Album}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Album> result = albumRepository
            ...
            .map(existingAlbum -> {
                if (album.getNombre() != null) {
                    existingAlbum.setNombre(album.getNombre());
                }
                if (album.getDescripcion() != null) {
                    existingAlbum.setDescripcion(album.getDescripcion());
                }
                if (album.getCantFotos() != null) {
                    existingAlbum.setCantFotos(album.getCantFotos());
                }
